 ...
 * @generated
 */
public class DistrictItemSemanticEditPolicy extends
		CityBaseItemSemanticEditPolicy {

	/**
	 * @generated
	 */
	public DistrictItemSemanticEditPolicy() {
	...
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (CityVisualIDRegistry.getVisualID(node)) {
			case DistrictDistrictPlacesCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit
						.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (CityVisualIDRegistry.getVisualID(cnode)) {
